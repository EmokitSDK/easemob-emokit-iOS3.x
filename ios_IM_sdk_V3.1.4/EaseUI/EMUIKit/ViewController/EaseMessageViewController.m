...
#warning EmoKit Add
        NSString *messageKey = [NSString stringWithFormat:@"%@", [[[model.fileLocalPath lastPathComponent] lastPathComponent] stringByDeletingPathExtension]];
        NSString *emo = [EMKEmoManager shareInstance].emoMessageDict[messageKey];
        if (emo.length > 0)
        {
            model.emoFromEmoKit = emo;
        }
...
#warning EmoKit Add
                BOOL isCurChat = [message.conversationId isEqualToString:self.conversation.conversationId];
                // 若不是当前聊天对象的聊天界面，这里不需要识别情绪，聊天列表识别
                if (!isCurChat)
                    return;
                
                __weak typeof(self) weakSelf = self;
                EaseMessageModel *messageModel = [[EaseMessageModel alloc] initWithMessage:message];
                BOOL isFile = [[NSFileManager defaultManager] fileExistsAtPath:messageModel.fileLocalPath];
                
                NSString *messageKey = [NSString stringWithFormat:@"%@", [[messageModel.fileLocalPath lastPathComponent] stringByDeletingPathExtension]];
                NSString *emo = [EMKEmoManager shareInstance].emoMessageDict[messageKey];
                BOOL isEmo = emo.length > 0 ? YES : NO;
                
                if (isFile && !isEmo)
                {
                    [[EMKEmoManager shareInstance] emoWithAMRFilePath:messageModel.fileLocalPath messageModel:messageModel completionHandler:^(NSString *emo, BOOL finished) {
...
{
#warning EmoKit Add
    id<IMessageModel> messageModel = [self.dataArray lastObject];
    if (messageModel.bodyType == EMMessageBodyTypeVoice)
    {
        NSString *amrPath = messageModel.fileLocalPath;
        NSFileManager *fileManager = [NSFileManager defaultManager];
        if ([fileManager fileExistsAtPath:amrPath])
        {
            __weak typeof(self) weakSelf = self;
            
            [[EMKEmoManager shareInstance] emoWithAMRFilePath:amrPath
