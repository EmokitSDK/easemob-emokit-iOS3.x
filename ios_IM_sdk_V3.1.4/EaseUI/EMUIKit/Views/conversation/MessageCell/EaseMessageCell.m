...
#warning EmoKit Add
    _emoLabel = [[UILabel alloc] init];
    _emoLabel.translatesAutoresizingMaskIntoConstraints = NO;
    _emoLabel.backgroundColor = [UIColor clearColor];
    _emoLabel.textAlignment = NSTextAlignmentCenter;
    _emoLabel.font = [UIFont systemFontOfSize:12];
    [_emoLabel sizeToFit];
    [self.contentView addSubview:_emoLabel];
...
#warning EmoKit Add
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.emoLabel attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.bubbleView attribute:NSLayoutAttributeTop multiplier:1.0 constant:-6]];
}
...
#warning EmoKit Add
                _emoLabel.text = model.emoFromEmoKit;
            }
                break;
            case EMMessageBodyTypeVideo:
            {
