...
#warning EmoKit Add
    ...
    BOOL isCurChat = NO;
    if (_chatVC) {
        isCurChat = [message.conversationId isEqualToString:_chatVC.conversation.conversationId];
    }
    // 若是当前聊天对象的聊天界面，这里不需要识别情绪，聊天界面识别
    if (_chatVC && isCurChat)
        return;
    
    
    if (!error) {
        EMFileMessageBody *fileBody = (EMFileMessageBody*)[message body];
        if([fileBody type] == EMMessageBodyTypeVoice){
